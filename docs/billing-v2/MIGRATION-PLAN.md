...
La clé d'idempotence du checkout V2 est unique sur `(customer_id, provider, environment, idempotency_key)` et la ligne `billing_v2_authoritative_checkout_requests` en est l'ancre persistée : elle porte la `selection_fingerprint` de la sélection reçue. Cette ligne est relue **avant toute création financière**. Si elle existe et que l'empreinte de sélection est identique, la demande est rejouée et le résultat existant est renvoyé (`Created: false`), sans second abonnement, SubscriptionChange, BillingEvent, outbox ni intention orpheline. Si l'empreinte diffère, la demande est refusée en `BILLING_V2_IDEMPOTENCY_KEY_REUSED_WITH_DIFFERENT_SELECTION`. L'insertion du registre n'utilise plus `INSERT IGNORE` : une course est tranchée par la contrainte unique, la transaction perdante est annulée puis rejouée depuis la ligne gagnante.
