...
export async function verifyHCaptcha(
  ...
  try {
    ...
    // On journalise le détail (error-codes, hostname) pour le diagnostic
    // serveur ET on remonte errorCodes au caller sans l'exposer au client.
    ...
    return { ok: false, code: "CAPTCHA_FAILED", errorCodes };
  } catch {
    logHCaptchaFailure({
      correlationId,
