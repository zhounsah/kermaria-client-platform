...
export function PublicStorefrontPage({
  ...
  const relatedLinks = resolveStorefrontPublicRelatedLinks(content.relatedLinks, selfServiceOrderable);
  return (
    <div className="services-page storefront-page">
      ...
      <section className="service-cta"><div><h2>Parlons de votre besoin.</h2><p>Un devis ou un audit permet de confirmer le périmètre, les prérequis et les limites avant mise en service.</p></div><Link className="button button-secondary" href={cta.href}>{cta.label}</Link></section>
    </div>
  );
}
