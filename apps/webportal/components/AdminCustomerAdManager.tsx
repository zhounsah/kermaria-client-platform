  ...
  async function searchObjects(kind: "users" | "groups") {
    ...
    if (result.ok) {
      ...
      if (objects === null) {
        ...
        setMessage({
          tone: "error",
          text: "Le format de réponse de la recherche Active Directory est invalide.",
        });
      } else if (kind === "users") {
...
async function submitMutation<TPayload>(
  ...
  async function handleLoadEffectiveGroups() {
    if (!selectedUser) {
      setMessage({
        tone: "error",
        text: "Sélectionnez un utilisateur avant de lister ses groupes.",
      });
      return;
    }
  ...
  async function handleRenameUser(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedUser) {
      setMessage({
        tone: "error",
        text: "Sélectionnez un utilisateur à renommer.",
      });
      return;
    }
  ...
  async function handleMoveUser(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedUser) {
      setMessage({
        tone: "error",
        text: "Sélectionnez un utilisateur à déplacer.",
      });
      return;
    }
  ...
  return (
    <div className="stack-panels">
      <SectionCard ariaLabel="État Active Directory du client" className="stack-panel">
        ...
        {statusError ? (
          <FormMessage title="État AD indisponible" tone="error">
            <p>{statusError}</p>
          </FormMessage>
        ) : null}
        {linksError ? (
        ...
        {message ? (
          <div ref={messageRef}>
            <FormMessage
              title={message.tone === "success" ? "Action terminée" : message.tone === "info" ? "Information" : "Échec"}
              tone={message.tone}
            >
              <p>{message.text}</p>
            </FormMessage>
      ...
      <div className="request-detail-layout">
        <SectionCard ariaLabel="Recherche utilisateurs AD">
          ...
          {usersSearchCount !== null ? (
            <p className="field-hint">
              {usersSearchCount} résultat(s) trouvé(s).
            </p>
          ) : null}
          {selectedUser ? (
