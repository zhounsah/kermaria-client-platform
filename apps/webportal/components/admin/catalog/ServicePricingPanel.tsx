...
import { classifyAdminPrice, formatAdminDateTime } from "@/lib/admin-catalog-presenters";
import type { BillingV2AdminPrice, BillingV2AdminService } from "@/lib/internal-api";
import { CatalogFeedback, CatalogField, adminCatalogStyles as styles } from "./AdminCatalogUi";
...
export function ServicePricingPanel({ asOf, service }: { asOf: string; service: BillingV2AdminService }) {
  ...
  async function publish(event: React.FormEvent, target: Target) {
    ...
    if (result) { setEditing(null); setRevision(EMPTY_REVISION); }
  }
  ...
  return <section className={styles.panel}>
    ...
    <div className={styles.stack}>{targets.map((target) => {
      ...
      const comparisonPrice = current.find((price) => price.billingCadence === revision.cadence && price.chargeTrigger === revision.trigger) ?? null;
      return <article className={styles.stack} key={target.key}>
        <div className={styles.sectionHeading}><div><h3>{target.label}</h3><p>{current.length} tarif(s) en vigueur · {scheduled.length} planifié(s) · {history.length} historique(s)</p></div><button className="button button-secondary button-small" onClick={() => { setEditing(target.key); setRevision({ ...EMPTY_REVISION, taxPercent: basisPointsToPercent(current[0]?.taxRateBasisPoints) }); }} type="button">Créer une nouvelle version</button></div>
        ...
        {editing === target.key ? <form className={styles.inlineForm} onSubmit={(event) => void publish(event, target)}>
          ...
          <CatalogField htmlFor={`cadence-${target.key}`} label="Cadence"><select id={`cadence-${target.key}`} onChange={(e) => setRevision({ ...revision, cadence: e.currentTarget.value })} value={revision.cadence}><option value="monthly">Mensuel</option><option value="one_time">Ponctuel</option></select></CatalogField>
          <CatalogField htmlFor={`trigger-${target.key}`} label="Déclencheur"><select id={`trigger-${target.key}`} onChange={(e) => setRevision({ ...revision, trigger: e.currentTarget.value })} value={revision.trigger}><option value="initial_subscription">Souscription initiale</option><option value="subscription_change">Changement de configuration</option></select></CatalogField>
          <CatalogField htmlFor={`tax-${target.key}`} label="TVA (%)" hint="Affichée en pourcentage, transmise en points de base."><input id={`tax-${target.key}`} inputMode="decimal" onChange={(e) => setRevision({ ...revision, taxPercent: e.currentTarget.value })} placeholder="20" value={revision.taxPercent} /></CatalogField>
          ...
          <div className={`${styles.fieldFull} ${styles.actionGroup}`}><button className="button button-secondary" onClick={() => setEditing(null)} type="button">Annuler</button><button className="button" disabled={command.busy} type="submit">Publier la nouvelle version</button></div>
        </form> : null}
