...
export async function POST(request: NextRequest) {
  ...
  if (isWebhookVerificationEnabled()) {
    ...
    console.warn(
      "PayPal webhook signature verification disabled via PAYPAL_WEBHOOK_VERIFY=false",
    );
  }
  ...
  if (!eventId || !eventType) {
    return NextResponse.json(
      { code: "INVALID_REQUEST", message: "Missing id or event_type." },
      { status: 400 },
    );
  }
  ...
  try {
    const response = await fetch(
      `${internalApiUrl}/internal/webhooks/paypal`,
      {
        method: "POST",
        cache: "no-store",
        ...
        body: JSON.stringify({
          eventId,
          eventType,
          resourceId: parsed.resource?.id ?? null,
          rawPayload: rawBody,
        }),
      },
    );
    if (!response.ok) {
      ...
      console.error(
        "PayPal webhook internal processing returned non-2xx:",
        response.status,
        text,
      );
      return NextResponse.json(
